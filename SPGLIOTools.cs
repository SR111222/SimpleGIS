...
namespace SimpleGIS
{
    internal class SPGLIOTools
    {
        #region 程序集方法

        internal static MyMapObjects.moMapLayer LoadSPGLLayer(string filePath)
        {
            try
            {
                //获取文件名称
                ...
                //读取spglx文件
                using (FileStream shx = new FileStream(filePath.Substring(0, filePath.IndexOf(".spgl")) + ".spglx", FileMode.Open, FileAccess.Read))
                {
                ...
                //读取spgl文件
                using (FileStream shp = new FileStream(filePath.Substring(0, filePath.IndexOf(".spgl")) + ".spgl", FileMode.Open, FileAccess.Read))
                {
                    using (BinaryReader br = new BinaryReader(shp))
                    {
                        //读取spgldbf文件
                        using (FileStream dbf = new FileStream(filePath.Substring(0, filePath.IndexOf(".spgl")) + ".spgldbf", FileMode.Open, FileAccess.Read))
                        {
        ...
        internal static void SaveSPGLLayer(MyMapObjects.moMapLayer map,string filePath)
        {
            FileStream fs = new FileStream(filePath, FileMode.Create);
            StreamWriter sw = new StreamWriter(fs);
            sw.WriteLine(map.Name);
            sw.WriteLine(map.ShapeType);
